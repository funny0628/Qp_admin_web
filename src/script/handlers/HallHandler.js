...
class HallHandler extends BaseHandler {
  ...
  //删除内部邮件
  email_delete(data,user_id) {
    return this.set_v1("/email_delete/" + user_id, data);
  }
  ...
  //修改大厅海报
  poster_set(data,user_id) {
    return this.set_v1("/poster_set/" + user_id, data);
  }

  //删除大厅海报
  poster_delete(data,user_id) {
    return this.set_v1("/poster_delete/" + user_id, data);
  }

  //增加大厅海报
  poster_add(data,user_id) {
    return this.set_v1("/poster_add/" + user_id, data);
  }
  //大厅海报类型查询列表
  poster_type_list(data,user_id) {
    return this.get_v1("/poster_type_list/" + user_id, data);
  }
  //大厅海报点击动作查询列表
  poster_act_list(data,user_id) {
    return this.get_v1("/poster_act_list/" + user_id, data);
  }
}
