<template>
  ...
  >
    <el-button v-has="'lhd_robot_config_save'" type="primary" @click="submitForm('ruleForm', 1)">保存</el-button
    ><el-button v-has="'lhd_robot_config_send'" type="primary" @click="submitForm('ruleForm', 2)"
      >发送到服务器配置</el-button
    >
    <div v-has="'lhd_robot_config_detail'">
    <!-- 头部 -->
    <div class="title">
    ...
    <div class="form">
      ...
      >
        ...
        <el-form-item label="下注和区域浮动值" prop="system_bet_area">
          <el-input
            style="width:200px"
...
export default {
  ...
  async created() {
    let { data } = await this.$http.HallFunConfig.Getlfdj_robot_control({
      key: "lfdj_robot_control.lua"
    });
    // console.log(data);
    ...
    // console.log(res);
    this.currentlist = res.lfdj_normal.robot_type_list;
    ...
    submitForm(formName, type) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          if (type === 1) {
            //put
            let { data } = await this.$http.HallFunConfig.Putlfdj_robot_control({
              keys: this.keys,
              values: JSON.stringify({
                lfdj_normal: { robot_type_list: this.currentlist }
              }),
              id: this.id
            });
            // console.log(data);
            ...
            let { data } = await this.$http.HallFunConfig.Postlfdj_robot_control({
              keys: this.keys,
              values: JSON.stringify({
                lfdj_normal: { robot_type_list: this.currentlist }
              }),
