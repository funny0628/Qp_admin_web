<template>
  ...
  >
  <div v-has="'bmbc_robot_config_detail'">
    <el-button v-has="'bmbc_robot_config_save'" type="primary" @click="submitForm('ruleForm', 1)">保存</el-button
    ><el-button v-has="'bmbc_robot_config_send'" type="primary" @click="submitForm('ruleForm', 2)"
      >发送到服务器配置</el-button
    >
    <!-- 头部 -->
    <div class="title">
    ...
    <div class="form">
      ...
      >
        ...
        <el-form-item label="下注区域比例" prop="system_bet_area">
          <el-input
            style="width:200px"
        ...
        <el-form-item label="上座率(百分比)" prop="vip_seat_rate">
          <el-input
            style="width:200px"
        ...
        <el-form-item label="离桌率(百分比)" prop="leave_rate">
          <el-input
            style="width:200px"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        leave_rate: [{ required: true, message: "不可以为空", trigger: "blur" }]
      },
      //当前游戏房间的所有数据
      currentlist: {},
      //匹配当前游戏的条件
  ...
  async created() {
    let { data } = await this.$http.HallFunConfig.Getbmbc_robot_control({
      key: "bmbc_robot_control.lua"
    });
    // console.log(data);
    ...
    // console.log(res);
    this.currentlist = res.bcbm_normal.robot_type_list;
    ...
    submitForm(formName, type) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          if (type === 1) {
            //put
            let { data } = await this.$http.HallFunConfig.Putbmbc_robot_control({
              keys: this.keys,
              values: JSON.stringify({
                bcbm_normal: { robot_type_list: this.currentlist }
              }),
              id: this.id
            });
            // console.log(data);
            ...
            let { data } = await this.$http.HallFunConfig.Postbmbc_robot_control({
              keys: this.keys,
              values: JSON.stringify({
                bcbm_normal: { robot_type_list: this.currentlist }
              }),
